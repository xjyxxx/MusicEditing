...
try {
    ...
    # Wait for UI process to exit (max ~90s)
    if ($waitPid -gt 0) {
    ...
    # Prepare source_dir from zip if needed
    ...
    # Never copy from inside install_root into itself
    ...
    # Best-effort cleanup bak (may fail if locked)
    try {
