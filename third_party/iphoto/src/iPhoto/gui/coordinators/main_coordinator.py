...
class MainCoordinator(QObject):
    ...
    def __init__(
        ...
        self._shortcut_manager = AppShortcutManager(
            ...
            next_item_cb=self.request_next_item,
            prev_item_cb=self.request_previous_item,
            parent=self,
        )
    ...
    def shutdown(self) -> None:
        ...
        # 被 MusicEditing 等宿主嵌入时，只拆图库，绝不能 quit 整个进程
        import os
        ...
        try:
            is_child_widget = window.parentWidget() is not None
        except Exception:  # noqa: BLE001
            pass
        if hosted or is_child_widget:
            self._logger.info(
                "MainCoordinator.shutdown: hosted/embedded — skip app.quit()"
            )
            return
