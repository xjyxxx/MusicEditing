...
models/lama.onnx   # scripts/download_lama_model.bat，不进 git
```

外部目录（不进 git）：`E:\FFmpegxuexi\onnxruntime-win-x64-gpu_cuda13-1.27.1\` 或 `E:\FFmpegxuexi\onnxruntime\`

| 脚本 | 作用 |
|------|------|
| `scripts/import_onnxruntime.bat` | 从 FFmpegxuexi 已解压目录复制 include/lib/bin（默认 GPU 1.27.1） |
| `scripts/setup_onnxruntime_x64.bat` | 优先本地 GPU 包，否则下载 CPU 包并导入 |
...
- `media_watermark_load_model(path)`
...
```
WatermarkPage（图片/视频 Tab）
  → RegionSelectorWidget 框选多矩形
  → MainViewModel.start_watermark_image / start_watermark_video
  → MediaBridge.watermark_inpaint_image / watermark_inpaint_video
  → media_cli watermark-inpaint-frames（一次加载模型）+ ffmpeg 抽帧/编码/混音
```

视频模式：在预览帧上框选固定位置水印 → 设定起止时间 → FFmpeg 抽帧 → LaMa 批量修复（单次 `media_cli` 进程）→ `h264_mf`（Windows）/ `libx264` 重编码并合并原音轨。

**性能说明：** 旧版逐帧调用 `watermark-inpaint` 会每帧重新加载约 200MB LaMa 模型（极慢）；现改为 `watermark-inpaint-frames` 只加载一次。长视频仍受 CPU LaMa 512×512 推理耗时限制；CUDA EP 尚未在 `WatermarkInpainter` 中启用。
...
| **离线批处理解码** | `VideoDecoder` CPU | ❌ 未启用 | `media_cli iterate` 仍为 CPU |
...
| **去水印 LaMa** | ONNX Runtime + OpenCV | ✅ UI 已接入 | `WatermarkPage`；视频 FFmpeg+逐帧 LaMa |
...
3. **P2** — `VideoDecoder` / `media_cli iterate` 硬解  
...
| 类 | 所在进程 | 用途 | 状态 |
|----|----------|------|------|
| `VideoDecoder` | `media_engine.dll` ← `media_cli.exe` | 探测元数据、逐帧遍历、抽缩略图 | 无播放态，打开→处理→关闭 |
| `VideoPlayerEngine` | `media_player.exe` | 实时拉帧、Seek、写 RGB 文件给 Python 显示 | 有 `lastTimestamp`、EOF、暂停等播放状态 |

调用链：

```
Python MediaBridge.probe_video / iterate_frames
  └─ subprocess: media_cli.exe probe|iterate ...
       └─ media_engine.cpp (C API)
            └─ VideoDecoder::open / iterateFrames / extractThumbnail
...
| `media_iterate_frames` | `open` → `iterateFrames` → 析构 | `iterate <path> [maxFrames]` |
| `media_extract_thumbnail` | `open` → `extractThumbnail` → 析构 | （API 已有，CLI 未单独暴露） |
...
| 中文路径 | 直接 `filePath.c_str()` | `pathUtf8ToNative()` 转 GBK |
...
media_iterate_frames(path, callback, userData)
media_extract_thumbnail(path, timestamp, rgb, size)
...
**iterate 命令：**
```
media_cli iterate <path> [maxFrames]
→ stdout (逐行):
...
| FFmpeg GPU 硬解（D3D11VA） | ✅ | 首页播放器 x64；`gpu_enabled` + 标题 `D3D11VA` |
...
| 批量导出剪辑 | ⏳ | UI 按钮已有，逻辑未写 |
| 4K 超分 | ⏳ | 占位页 |
| 去水印 | ✅ | `WatermarkPage` 图片/视频；`MediaBridge` + LaMa ONNX |
| 授权/卡密 | ⏳ | network.py 预留 |
...
详见 **§3.6**。已完成播放器 D3D11VA 硬解。后续：

1. **llama 推理 GPU**：`-DGGML_CUDA=ON` 重编 → `HighlightAnalyzer` 按 `use_gpu` 设置 `LlmConfig.n_gpu_layers`
2. **VideoDecoder 硬解**：`media_cli iterate` 同样接入 `HwAccelContext`
3. **个人中心 UI**：调用 `set_gpu_enabled()` 切换硬解/CUDA 偏好
4. **OpenCV CUDA**（可选）：减少硬解后的 CPU 滤镜开销
